...
export SA_SECRET=$(kubectl -n default get sa vault-auth -o jsonpath='{.secrets[0].name}')
export SA_TOKEN=$(kubectl -n default get secret "${SA_SECRET}" -o jsonpath='{.data.token}' | base64 --decode)
...
if [[ -z "${DB_IP}" ]]; then echo "Database IP must be specified to enable auth" && exit 1; else kubectl exec -ti vault-0 -n vault -- sh -c "vault write database/config/postgres plugin_name=postgresql-database-plugin connection_url='postgresql://{{username}}:{{password}}@${DB_IP}:5432/postgres?sslmode=disable' allowed_roles=* username=${DB_USERNAME} password=${DB_PASSWORD}"; fi
